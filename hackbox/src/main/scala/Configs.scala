...
class HackBoxConfig extends ChiselConfig {
  override val topDefinitions:World.TopDefs = {
    (pname,site,here) => pname match {
      ...
      case BuildRoCC => Some(() => (Module(new HackBoxAccumulator, { case CoreName => "HackBoxAccum" })))
    }
  }
...
class HackBoxVLSIConfig extends ChiselConfig(new HackBoxConfig ++ new DefaultVLSIConfig)
class HackBoxFPGAConfig extends ChiselConfig(new HackBoxConfig ++ new DefaultFPGAConfig) 
class HackBoxCPPConfig extends ChiselConfig(new HackBoxConfig ++ new DefaultCPPConfig)
